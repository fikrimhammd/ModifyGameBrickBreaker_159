...
class Brick(GameObject):
    COLORS = {1: '#1F618D', 2: '#48C9B0', 3: '#F4D03F'}  # #command: Warna brick berdasarkan jumlah nyawa
...
class Game(tk.Frame):
    def __init__(self, master):
        ...
        self.canvas = tk.Canvas(self, bg='#D7EAF5',  # #command: Membuat canvas dengan warna latar
                                width=self.width,
                                height=self.height)
    ...
    def draw_text(self, x, y, text, size='20'):
        ...
        return self.canvas.create_text(x, y, text=text, font=font)  # #command: Menambahkan teks ke canvas
